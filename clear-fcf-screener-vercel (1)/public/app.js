...
const STORAGE_KEY = "clear-fcf-auto-v2";
...
function tableHtml(results, title) {
  ...
  return `<div class="results-shell">
    ...
    <div class="table-scroll"><table><thead><tr>
      <th>Company</th><th>Status</th><th>Price</th><th>Market cap</th><th>Revenue</th><th>Conservative P/nFCF</th><th>EV/nFCF</th><th>Normalized FCF yield</th><th>Avg FCF yield*</th><th>History</th><th>Reliability</th><th>Net debt/nFCF</th><th>Share change</th><th>SBC/revenue</th>
      ...
      <td>${escapeHtml(row.usableYears)} years</td><td>${escapeHtml(row.reliability)}</td><td>${multiple(row.netDebtToNfcf)}</td><td>${percent(row.shareChange)}</td><td>${percent(row.latestSbcToRevenue)}</td>
...
function exportCsv(results) {
  const fields = ["ticker", "companyName", "status", "price", "marketCap", "latestRevenue", "averageAnnualFcf", "priceToNfcf", "evToNfcf", "fcfYield", "averageFcfYield", "latestFcfYield", "usableYears", "reliability", "netDebtToNfcf", "shareChange", "latestSbcToRevenue"];
...
function renderReport(report) {
  ...
  <div class="report-grid">
    ...
    <div class="metric"><span>Conservative P/nFCF</span><strong>${multiple(v.priceToNfcf)}</strong></div>
  ...
  <div class="report-card"><h3>Normalization inputs</h3><div class="formulas">
    <div class="formula"><span>Aggregate-margin method</span><strong>${compact(n.marginNormalizedFcf, true)}</strong><small>${percent(n.aggregateMargin)} = ${compact(n.totalFcf, true)} total FCF ÷ ${compact(n.totalRevenue, true)} total revenue</small></div>
    <div class="formula"><span>Median-margin method</span><strong>${compact(n.medianNormalizedFcf, true)}</strong><small>${percent(n.medianMargin)} median margin × ${compact(n.latestRevenue, true)} latest revenue</small></div>
    <div class="formula"><span>Conservative case</span><strong>${compact(n.conservativeFcf, true)}</strong><small>Lower positive estimate</small></div>
  </div><p class="footnote">* Average FCF yield means historical average annual FCF divided by today's market capitalization. It is not an average of historical market yields.</p></div>
  <div class="report-card"><h3>Calculation audit</h3><div class="audit-list">
    ...
    <div><span>Aggregate FCF margin</span><code>${compact(n.totalFcf, true)} ÷ ${compact(n.totalRevenue, true)} = ${percent(n.aggregateMargin)}</code></div>
    ...
    <div><span>Conservative P/nFCF</span><code>${compact(v.marketCap, true)} ÷ ${compact(n.conservativeFcf, true)} = ${multiple(v.priceToNfcf)}</code></div>
    <div><span>Normalized FCF yield</span><code>${compact(n.conservativeFcf, true)} ÷ ${compact(v.marketCap, true)} = ${percent(v.fcfYield)}</code></div>
    ...
    <div><span>EV/nFCF</span><code>${compact(v.enterpriseValue, true)} ÷ ${compact(n.conservativeFcf, true)} = ${multiple(v.evToNfcf)}</code></div>
  </div></div>
