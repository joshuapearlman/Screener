...
function normalize(rows) {
  ...
  if (!usable.length || !finite(latestRevenue)) return { usableYears: 0, latestRevenue, latestReportedFcf: null, totalRevenue: null, totalFcf: null, averageAnnualFcf: null, averageMargin: null, medianMargin: null, averageMedianGap: null, marginAgreement: false, outlierYears: [], normalizedFcf: null };
  ...
  const averageMedianGap = finite(medianMargin) && Math.abs(medianMargin) > 1e-12
    ? Math.abs(averageMargin - medianMargin) / Math.abs(medianMargin)
    : Math.abs(averageMargin - (medianMargin || 0)) <= 1e-12 ? 0 : null;
  const marginAgreement = finite(averageMedianGap) && averageMedianGap <= 0.2;
  const absoluteDeviations = usable.map((row) => Math.abs(row.fcfMargin - medianMargin));
  const medianAbsoluteDeviation = median(absoluteDeviations);
  const outlierYears = usable.filter((row) => {
    const deviation = Math.abs(row.fcfMargin - medianMargin);
    if (medianAbsoluteDeviation > 1e-12) return 0.6745 * deviation / medianAbsoluteDeviation > 3.5;
    return deviation > 0.05;
  }).map((row) => ({ year: row.year, margin: row.fcfMargin }));
  const normalizedFcf = averageMargin * latestRevenue;
  return {
    usableYears: usable.length, latestRevenue, latestReportedFcf: usable[0].fcf,
    totalRevenue, totalFcf, averageAnnualFcf, averageMargin, medianMargin,
    averageMedianGap, marginAgreement, medianAbsoluteDeviation, outlierYears, normalizedFcf,
...
async function analyzeTicker(rawTicker, options = {}) {
  ...
  const nfcf = normalized.normalizedFcf;
  const valuation = {
  ...
  if (!normalized.marginAgreement) warnings.push(`Average and median annual FCF margins differ by more than 20%${finite(normalized.averageMedianGap) ? ` (${round(normalized.averageMedianGap * 100, 1)}%)` : ""}; this stock cannot qualify automatically.`);
  if (normalized.outlierYears.length) warnings.push(`Extreme annual FCF margin detected in: ${normalized.outlierYears.map((item) => item.year).join(", ")}.`);
  ...
  const passes = !isFinancial && hasUsdQuote && valuationSane && normalized.marginAgreement && normalized.usableYears >= 6 && quality.reliability !== "Low" && finite(valuation.priceToNfcf) && valuation.priceToNfcf <= 15 && (!finite(valuation.netDebtToNfcf) || valuation.netDebtToNfcf <= 5) && (!finite(quality.shareChange) || quality.shareChange <= 0.2);
  ...
  return {
    ...
    methodology: {
      ...
      normalizedFcf: "Simple average of annual FCF margins × latest reported annual revenue",
...
function summary(report) {
  return {
    ...
    normalizedFcf: round(report.normalized.normalizedFcf), usableYears: report.normalized.usableYears,
    averageFcfMargin: round(report.normalized.averageMargin, 4), medianFcfMargin: round(report.normalized.medianMargin, 4),
    averageMedianGap: round(report.normalized.averageMedianGap, 4), marginAgreement: report.normalized.marginAgreement,
