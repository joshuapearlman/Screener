...
function normalize(rows) {
  ...
  if (!usable.length || !finite(latestRevenue)) return { usableYears: 0, latestRevenue, aggregateMargin: null, medianMargin: null, marginNormalizedFcf: null, medianNormalizedFcf: null, conservativeFcf: null };
  ...
  return {
    usableYears: usable.length, latestRevenue, aggregateMargin, medianMargin,
...
async function analyzeTicker(rawTicker, options = {}) {
  ...
  const shares = extracted.latest.sharesOutstanding?.val ?? rows.find((row) => finite(row.shares))?.shares ?? null;
  ...
  const valuation = {
    price, shares, marketCap, debt, cash, netDebt, enterpriseValue,
  ...
  const passes = !isFinancial && hasUsdQuote && normalized.usableYears >= 6 && quality.reliability !== "Low" && finite(valuation.priceToNfcf) && valuation.priceToNfcf <= 15 && (!finite(valuation.netDebtToNfcf) || valuation.netDebtToNfcf <= 5) && (!finite(quality.shareChange) || quality.shareChange <= 0.2);
...
function summary(report) {
  return {
    ...
    price: round(report.valuation.price), priceToNfcf: round(report.valuation.priceToNfcf),
...
async function analyzeBatch(tickers, options = {}) {
  ...
  async function worker() {
    while (index < tickers.length) {
      ...
      try { reports[ticker] = await analyzeTicker(ticker, options); }
...
module.exports = { annualRows, normalize, computeQuality, analyzeTicker, analyzeBatch, summary };
